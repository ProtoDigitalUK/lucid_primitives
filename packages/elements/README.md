...
JavaScript store modules. Elements uses SolidJS signals for reactivity while
...
`standardHandlers` includes refs, bindings, effects, loops, events, DOM
updates, and focus traps. Handlers can instead be imported individually when a
smaller feature set is required.
...
```ts
import { createSignal, storeModule } from "@lucidclient/elements";
...
Global effects run whenever the effects handler is registered. Duplicate
...
Use `sync` after adding markup outside Elements:
...
```ts
Elements.refresh();
Elements.refresh("storeScope");
Elements.destroy();
```

`destroy` resets the running instance so it can be started again. Registered
store modules and handler definitions remain available.
...
Handler attribute names are owned by their handler definitions rather than by
core configuration.
